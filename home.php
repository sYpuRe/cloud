...
<?php
		$user=$_SESSION['user1'];
		$pass=$_SESSION['pass1'];
		$pr=mysqli_query($conn,"SELECT * from login where user='$user' and pass='$pass'");
		while($x=mysqli_fetch_array($pr))
		{
    ...
    function ShowLoading(e) {
		...
		if(isset($_POST['up']))
		{
			if(empty($_FILES['img']['tmp_name'])){echo"select file!";}
					...
					$target="pic/".$pic;
					mysqli_query($conn,"UPDATE login SET pic='$pic' WHERE user='$user'");
